...
import Formation from '../../../components/GameDetail/Formation';
import { getPlayersByGameId, IPlayers } from '../../../api/firebase/players';
...
const GameDetail = () => {
  ...
  const ListItems = [
    { name: 'Sumário', index: 0, component: <Summary teamHomeId={game?.teamhome.api} summaries={summaries} /> },
    { name: 'Estatisticas', index: 1, component: <Statistics homeTeamId={game?.teamhome.api} statistics={statistics} /> },
    { name: 'Formações', index: 2, component: <Formation homeTeamId={game?.teamhome.api} summaries={summaries} players={players} /> },
    { name: 'Classificação', index: 3, component: <Standings standings={standings} /> },
    { name: 'Videos', index: 4, component: <Videos videos={videos} /> },
    { name: 'MVP', index: 5, component: <ManOfTheMatch handleRefreshManofTheMatch={() => { }} manOfTheMatch={manOfTheMatch} lineups={lineups} /> },
  ];
  ...
  useEffect(() => {
    const getSummaries = async () => {
      ...
      const playersdata = await getPlayersByGameId(id)
      setPlayers(playersdata)
      console.log("PlayersData:::::::::", playersdata, id)
  ...
  return <SafeAreaView className={`bg-bgauth  flex-1 h-full justify-start items-center`}>
    ...
    <Animated.View
      ...
      />
      <View className='w-full pl-4 pr-4 flex-1'>

        <View className='w-full mt-3 flex-row justify-between items pr-4 pl-4 bg-white_20 rounded-md h-11'>
              ...
              return (
                ...
                >
                  <Text
                    ...
                    className={`text-white text-lg ${index !== item.index && 'opacity-50'} ${Platform.OS === 'ios' && 'pt-3'} font-dinRegular`}
                  >{item.name}</Text>
                </TouchableOpacity>
              )
            })}
        ...
        >
          <ScrollView
            contentContainerStyle={{ width: Dimensions.get('window').width, flexGrow: 1 }}>
            {summaries ?
              summaries.length > 0 ?
          ...
          <ScrollView contentContainerStyle={{ width: Dimensions.get('window').width, flexGrow: 1 }}>
            {videos ?
              ...
              :
              <View className='w-full h-full justify-center mt-14 items-center'>
          ...
          <ScrollView contentContainerStyle={{ width: Dimensions.get('window').width, flexGrow: 1 }}>
            {players ? players.length > 0 ?
              <Formation homeTeamId={game.teamhome.api} players={players} summaries={summaries}></Formation> : <></> : <></>}
          </ScrollView>
          <ScrollView contentContainerStyle={{ width: Dimensions.get('window').width, flexGrow: 1 }}>
            {standings ?
              <Standings standings={standings}></Standings>
              :
              <View className='w-full h-full ml-[-10] justify-center mt-14 items-center'>
                <Image source={require('../../../assets/icons/calendarcheck.png')}></Image>
                <Text className='text-lg text-white font-bold font-dinLight'>Disponível no dia de jogo</Text>
              </View>
            }
          </ScrollView>
          <ScrollView nestedScrollEnabled={true} contentContainerStyle={{ width: Dimensions.get('window').width, flexGrow: 1, paddingBottom: 50 }}>
            {lineups ?
              lineups.length > 0 ? <ManOfTheMatch handleRefreshManofTheMatch={handleRefreshManofTheMatch} manOfTheMatch={manOfTheMatch} lineups={lineups} ></ManOfTheMatch>
                : <View className='w-full h-40 justify-center mt-14 items-center'>
                  <Image source={require('../../../assets/icons/calendarcheck.png')}></Image>
                  <Text className='text-lg text-white font-bold font-dinLight'>Disponível no dia de jogo</Text>
                </View> :
              <View className='w-full h-40 justify-center mt-14 items-center'>
