...
import {
  IStastistics,
  getStastisticsByGameId,
} from "../../../api/firebase/stastics";
import {
  IStandings,
  getStandingsByCompetation,
} from "../../../api/firebase/standings";
...
import {
  IManOfTheMatch,
  getManOfTheMatch,
} from "../../../api/firebase/manOfTheMatch";
...
const GameDetail = () => {
  ...
  const subscribeGame = async (game: IGame) => {
    ...
    if (nextGame.api === game.api) {
      ...
      const subscriberSummary = firebase
        .firestore()
        .collection("summaries")
        .where("GameId", "==", game.api)
        .onSnapshot((documentSnapshot) => {
          const summary = documentSnapshot.docs.map((doc) =>
            doc.data(),
          ) as ISummary[];
          setSummaries(summary);
        });
      const subscriberStastisics = firebase
        .firestore()
        .collection("stastistics")
        .where("GameId", "==", game.api)
        .onSnapshot((documentSnapshot) => {
          const stastistics = documentSnapshot.docs.map((doc) =>
            doc.data(),
          ) as IStastistics[];
  ...
  if (!summaries && !game) {
    return (
      <SafeAreaView
        className={"bg-bgauth  flex-1 h-full justify-start items-center"}
      >
        <Animated.View
                  ...
                  <TouchableOpacity
                    onPress={() => {
                      scrollViewRef.current?.scrollTo({
                        x: Dimensions.get("window").width * item.index,
                        animated: true,
                      });
                    }}
                    className={`justify-center ${index === item.index && " border-b-2 border-b-white"}  flex mr-2 items-center`}
                  >
                    <Text
                      onPress={() => {
                        scrollViewRef.current?.scrollTo({
                ...
                summaries.length > 0 ? (
                  ...
                  <View className="w-full h-40 ml-[-10] justify-center mt-14 items-center">
                    <Text className="text-lg text-white font-bold font-dinLight">
                      Qual o teu prognostico para o Jogo ?{" "}
                    </Text>
                    <TouchableOpacity
                      onPress={() => openModal()}
                      activeOpacity={0.7}
                    ...
                    >
                      <Text
                        className={`text-titleauth font-dinBold ${Platform.OS === "ios" ? "mt-3" : "mt-0"} text-sm`}
                      >
                        ENVIAR
                      </Text>
                    </TouchableOpacity>
          ...
          <View className="bg-bgauth  h-full rounded-t-xl  w-full round-md   shadow-xl">
            ...
            >
              <View className="bg-bgauth  h-[390px] rounded-t-xl  w-full round-md  p-6 shadow-xl">
                ...
                />
                <Text className="font-dinLight self-center text-3xl leading-[42px] text-white">
                  MATCH
                  <Text className="font-dinBold ext-3xl leading-[40px] text-white">
                    DAY
                  </Text>
                </Text>
                <GamePrognosis
