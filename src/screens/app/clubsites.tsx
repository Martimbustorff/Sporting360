  ...
  return (
    ...
    >
      ...
      >
        ...
        <Text className=" pl-4 pr-4 font-dinLight text-base mb-6 leading-1 text-white">
          Aqui podes encontrar ligações directas para os sites oficiais do
          clube.{" "}
        </Text>
        <ScrollView>
          <Animated.View
            entering={FadeIn.delay(600)}
            className="flex justify-end flex-col flex-1  gap-4"
          >
                ...
                >
                  <View className="flex flex-row border-b border-b-white pb-3  gap-2 justify-between items-center">
                    <View className="flex flex-row  justify-center pb-3 items-center">
                      <Image
                        resizeMode="contain"
                        className="h-8 w-6 mr-2 object-contain"
                        source={item.image}
                      />
                      <Text
                        className={`font-dinBold ${Platform.OS === "ios" ? "h-6 mt-4" : "mt-0"} text-white text-sm`}
                      >
                        {item.title}
                      </Text>
                    </View>
