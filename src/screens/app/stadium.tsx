...
export const Stadium = () => {
  ...
  const getEdges = () => {
    "worklet";
    let pointX = (BOX_WIDTH * scaleValue.value - VISIBLE_AREA_WIDTH) * -1;
    const pointY = (BOX_HEIGHT * scaleValue.value - VISIBLE_AREA_HEIGHT) * -1;
  ...
  return (
    ...
    >
      ...
      <View className="flex-col w-full h-40 p-4">
        <View
          className={`flex flex-row ${Platform.OS === "android" ? "h-5" : "h-3"}  mb-12 gap-4 w-full justify-start items-center`}
        >
          <Image
            className=""
          ...
          />
          <Text
            className={`font-bold text-center  h-full text-sm text-white font-dinBold`}
          >
            MUSEU SPORTING
          </Text>
        </View>
