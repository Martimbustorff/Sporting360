...
const GameboxList = () => {
  ...
  const handleGetGameboxes = async () => {
    ...
    try {
      ...
      for (const iterator of gameboxListCollections.docs.sort(
        (x) => x.data.order,
      )) {
    ...
    >
      ...
      {gameboxes.length >= 6 && (
        <View className="w-full bg-white_20 h-8  justify-center items-center">
          <Text
            className={`text-white font-dinBold ${Platform.OS === "ios" && "pt-2"}  text-sm `}
          >
              ...
              return (
                <OpacityDecorator>
                  <TouchableOpacity
                    ...
                    activeOpacity={0.7}
                    className="flex w-full h-10 mb-4 justify-between flex-row"
                  >
                    <View className="flex flex-row items-center justify-center ">
                      {!item.default && (
                        <Image
                          className="h-7 mt-[-15] mr-2 w-7"
                          source={require("../../../assets/icons/Menu.png")}
                        />
                      )}
                      <Text
                        className={`text-base ${Platform.OS === "android" ? "h-7 mt-[-15]" : "h-7"}  font-dinBold text-white`}
                      >
                        {item.gameboxName}
                      </Text>
                    </View>
        ...
        {gameboxes.length < 6 && (
          ...
          >
            <Image
              className=" h-5 w-5"
              resizeMode="contain"
              source={require("../../../assets/icons/arrowplus.png")}
            />
          </TouchableOpacity>
        )}
      </View>
