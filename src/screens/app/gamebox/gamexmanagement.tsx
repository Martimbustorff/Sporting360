...
const GameboxMangement = () => {
  ...
  const handlePress = async () => {
    ...
    try {
      ...
      const numberEncprypt = CryptoJS.AES.encrypt(
        String(`${user.uuid}_${gameboxNumber.trim()}`),
        passphrase,
      ).toString();
  ...
  const handleGetGameboxById = async (id: String) => {
    ...
    if (doc) {
      ...
      setGameboxNumber(
        decryptGameBox(gameboxfirebase.gameboxNumber, user.uuid),
      );
  ...
  const handleDelete = async () => {
    try {
      firestore().collection("gamebox").doc(route.params?.id).delete();
      ...
      showMessage({
        message:
          "Não foi possivel eliminar a Gamebox, Por Favor tente novamente mais tarde",
        type: "success",
        color: "#fff",
        style: { height: 120, paddingTop: Platform.OS === "ios" ? 30 : 60 },
        titleStyle: { fontFamily: "DinBold", lineHeight: 20 },
  ...
  const handleGetGameboxes = async () => {
    ...
    try {
      const gameboxListCollections = await firestore()
        ...
        .get();
      const gameboxfirebase = defaultGamebox(user)
        ? [defaultGamebox(user)]
        : [];
    ...
    return (
      ...
      >
        ...
        >
          ...
          <View
            className={`mt-0  p-6 justify-center items-center   flex flex-1 w-full`}
          >
      ...
      >
        ...
        <View className={`mt-0  p-6   flex flex-1 w-full`}>
          <Animated.View
            ...
            className="flex flex-[0.9] w-full"
          >
            <View className="w-full flex  flex-row">
              {gameboxReaded !== null ? (
                gameboxReaded.gameboxNumber ? (
                  <Input
                    defaultValue={gameboxReaded.gameboxNumber}
                  ...
                  <Input
                    defaultValue={
                      gamebox
                        ? decryptGameBox(gamebox?.gameboxNumber, user.uuid)
                        : ""
                    }
                    keyboardType="number-pad"
                    autoCapitalize="none"
              ...
              >
                <Image
                  className="h-5 object-contain w-5 "
                  source={require("../../../assets/icons/qrcode.png")}
                />
              </TouchableOpacity>
            </View>
            <Input
              defaultValue={gamebox?.gameboxName}
