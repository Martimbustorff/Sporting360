...
const Home = () => {
  ...
  const handleGetGameboxes = async () => {
    try {
      const storage = new MMKV();
      const defaultGameboxInsert = storage.getString(
        "user.defaultGameboxInsert",
      );
      if (appversion < 5.4) {
        try {
          await firestore()
      ...
      const gameboxListCollections = await firestore()
        .collection("gamebox")
      ...
      if (user) {
        showMessage({
          message:
            "Não foi possível obter Gamebox! Por favor tente mais tarde.",
          type: "warning",
          color: "#D3D3D3",
          style: { height: 120, paddingTop: Platform.OS === "ios" ? 30 : 60 },
          titleStyle: { fontFamily: "DinBold", lineHeight: 20 },
  ...
  return (
    <View className="bg-background  h-full w-full">
      ...
      <Animated.View
        ...
        className="flex flex-col flex-[0.4] justify-between p-8 pt-16 "
      >
        {/* Header Component */}
        <HomeHeader navigation={navigation} user={user} />
      </Animated.View>
      {nextGame && (
        <View className="flex flex-[0.8]">
          <PrincipalGameHome
      ...
      <Animated.View
        entering={FadeIn.delay(1200)}
        className="flex justify-end flex-col flex-[0.9]  "
      >
        {/* Menu Items */}
        {Urls.map((item, index) => {
          return (
        ...
        <View
          className={` mb-16 mt-0 flex-col justify-center    w-full rounded-t-[48px] p-8 `}
        ></View>
        {/* Gamebox Section */}
        {user && gameboxes.length > 0 ? (
          <Animated.View
            style={bootomBackgroundStyle}
            className={`bg-white absolute flex-row    w-[100%] rounded-t-[48px] pr-8 pl-8 pb-8 pt-4 `}
          >
            {/* Gamebox Cards */}
            <View
              className={`w-full ${open ? "flex-col" : "flex-col"} justify-between`}
              onPress={() => handleOpenGameBoxWithFill()}
            >
              <View className="flex flex-row w-full justify-between items-center">
                <Text className="font-dinBold text-titleauth text-xl mt-[4] mb-2">
                  GAMEBOX DIGITAL
                </Text>
          ...
          <Animated.View
            style={bootomBackgroundStyle}
            className={`bg-white absolute flex-col     w-[100%] rounded-t-[48px] pt-4 pl-8 pr-8 pb-8 `}
          >
            {/* Gamebox Management Section */}
            <Pressable onPress={() => handleOpenGameBox()}>
              <View className="flex flex-row w-full justify-between items-center">
                <Text className="font-dinBold text-titleauth text-xl  ">
                  GAMEBOX DIGITAL
                </Text>
              ...
              <>
                {open && (
                  ...
                  >
                    <Text
                      className={`text-white font-dinBold ${Platform.OS === "ios" ? "mt-3" : "mt-0"} text-lg`}
                    >
                      DIGITALIZAR
                    </Text>
                  </TouchableOpacity>
                ...
                >
                  <Text className="text-black  text-sm  mb-4 font-dinRegular ">
                    ...
                    <Text className="text-titleauth  leading-5   text-sm font-dinBold">
                      {""} insere manualmente
                    </Text>{" "}
                    o número.
                  </Text>
