...
const Home = () => {
  ...
  return (
    <View className="bg-background  h-full w-full">
      ...
      <Animated.View entering={FadeIn.delay(1200)} className='flex flex-col flex-[0.2] justify-between p-8 pt-16 '>
        {/* Header Component */}
        <HomeHeader navigation={navigation} user={user} />
      </Animated.View>
      {nextGame ?
        <View className="flex flex-[0.3] pb-2">
          <PrincipalGameHome
          ...
          />
        </View> : <View className="flex flex-[0.3]"></View>
      }
      <Animated.View entering={FadeIn.delay(1200)} className='flex justify-between flex-col flex-[0.5]  '>
        {/* Menu Items */}
        {Urls.map((item, index) => {
          return <HomeMenuItem
            Urls={Urls}
        ...
        <View className={` mb-12 mt-0 flex-col w-full rounded-t-[28px] p-8 `}></View>
        {/* Gamebox Section */}
        {user && gameboxes.length > 0 ? (
          ...
          <View className="justify-end">
            <Animated.View style={bootomBackgroundStyle} className={`bg-white absolute flex-col w-[100%] rounded-t-[28px] pt-4 pl-8 pr-8 pb-4 `}>
