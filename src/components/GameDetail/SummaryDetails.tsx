...
const SummaryDetail: React.FC<ISummaryDetailPage> = ({
  summary,
  teamHomeId,
}) => {
  if (summary.type === "Var") {
  ...
  if (Number(teamHomeId) === Number(summary.team.api)) {
    return (
      <View className="w-auto mt-4 mb-2 flex-row items-center gap-4 justify-start">
        <Text className="text-white font-dinBold  top-1 text-base">
          {summary.time}´
        </Text>
        ...
        <Text className="text-white font-dinLight top-1 text-base">
          {summary.player.name}
        </Text>
      </View>
    );
  ...
  return (
    <View className="w-auto mt-4 mb-2 flex-row items-center gap-4 justify-end">
      <Text className="text-white font-dinLight top-1 text-base">
        {summary.player.name}
      </Text>
      <View className="bg-white w-8 h-8 rounded-full p-3 justify-center items-center">
