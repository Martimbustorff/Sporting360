...
const ManOfTheMatch: React.FC<IManOfTheMatchProps> = ({
  lineups,
  manOfTheMatch,
  ...
  const choices: Array<IChoice> =
    lineups.length == 0
      ? []
      : [
          ...lineups.filter((x) => x.teamId == "228")[0].startXI,
          ...lineups.filter((x) => x.teamId == "228")[0].substitutes,
        ]
          ...
          .map((k) => {
            return {
              ...
              votes: !manOfTheMatch
                ? 0
                : manOfTheMatch.filter((x) => x.playerId === k.playerId).length,
              choice: k.name,
            } as IChoice;
          });
  const [modalVisible, setModalVisible] = useState(false);
  return (
    <View className="flex flex-col justify-center w-full pr-10 pl-4">
      {lineups.length === 0 && (
        <View className="w-[85%] h-40 justify-center mt-0 items-center">
          <Image
            source={require("../../assets/icons/calendarcheck.png")}
          ></Image>
          <Text className="text-lg text-white font-bold font-Poppins font-base">
      ...
      {lineups.length > 0 && (
        <>
          <Text className="text-lg mt-4  text-white font-bold font-dinBold font-base">
            🦁 MVP
          </Text>
          {!userId && (
            <Text className="text-xs mt-2  text-white font-bold font-dinBold font-base">
              Para votar precisar estar autenticado
            </Text>
          )}
        </>
      )}
      ...
      <RNPoll
        ...
        hasBeenVoted={
          !manOfTheMatch
            ? false
            : manOfTheMatch.filter((x) => x.userId === userId).length > 0
        }
        onPress={() => {
          if (!userId) {
            navigate("Login");
