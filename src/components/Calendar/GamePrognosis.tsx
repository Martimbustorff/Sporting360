...
return  <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
   <View className='w-full pb-4 mt-3 h-60 '>
      <Text className='mb-3 text-white font-bold text-dinCondensed self-center'>
        {competation}
      </Text>
    <View className='w-full flex-row mb-3 justify-between'>
      <View className='w-1/3 mb-3'>
        <View style={{backgroundColor:'#00835B'}} className='  left-0 justify-center mt-2  items-center rounded-full w-[40%]'>
                <Text className={`font-dinCondensed  text-sm  text-white ${Platform.OS === 'ios' && ' h-full mt-[15]'}`}>J{jorney.trim()}</Text>
        </View> 
      </View>
    
      <View className='w-1/3  mb-3 justify-center items-center'>
    ...
    <View className='w-full flex-row  mt-[-10]  mb-0 justify-between '>
        <View className='flex justify-center flex-col items-center w-1/3  gap-2'>
            ...
            <Text className='font-dinLight 
                w-20
                text-center h-15 flex-wrap text-sm pt-1 mt-2 mb-2 text-white'>
              {hometeam}
            </Text>
        </View>
        <View className='flex flex-col w-1/3 items-center gap-2'>
          <View className=' justify-center items-center'>
              <View className='w-22 flex-row justify-between items-center'>
                  <TextInput 
                  keyboardType='number-pad'
                  className='bg-white w-11  mr-2 ml-2 text-bold font-dinCondensed text-5xl  text-center leading-[15px] h-20 rounded-md '>
                  </TextInput>
                  <TextInput 
                  onChangeText={(text)=>text ? Keyboard.dismiss() : null}
                  keyboardType='number-pad'
                  className='bg-white ml-2 w-11 text-bold font-dinCondensed text-5xl  text-center leading-[15px] h-20 rounded-md'/>
              </View>
          </View>
        </View>
