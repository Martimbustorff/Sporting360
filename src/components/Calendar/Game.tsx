...
const Game = ({
  ...
  function formatHour() {
    ...
    return formattedDate === "0:00" || formattedDate === "1:00"
      ? "N/D"
      : formattedDate;
  }
      ...
      <View className="w-full pb-3   pl-4  mt-6  border-b border-white_20 h-auto pr-4">
        <View className="w-full h-5 flex-row mb-2 items-center justify-between">
          <View
            ...
            className="   justify-center  items-center rounded-full top-[-3] h-4 w-9"
          >
            <Text
              className={`font-dinBold top-[-1]  text-xs  text-white ${Platform.OS === "ios" && " h-full mt-[10]"}`}
            >
              J{!jorney ? "1" : jorney.trim()}
            </Text>
          </View>
          <View className="w-full justify-center items-center ">
            <Text
              className={`font-dinLight   ${isLast ? "ml-[-86]" : "ml-[-90]"}  text-base h-5 text-white `}
            >
              {formatDate()}
            </Text>
          </View>
