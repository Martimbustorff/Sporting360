...
const HomeHeader = ({ navigation, user }) => {
  ...
  return (
    <View className='flex flex-col items-stretch justify-between h-full'>
       <View className=' flex-row justify-between items-center w-[100%] h-[20%]'>
              <View className="h-full items-center justify-center pt-2">
                <Text className={`font-dinBold ${Platform.OS === 'ios'? 'leading-10' : 'h-10' } pt-1 text-white text-4xl`}>S360</Text>
              </View>
      ...
      <TouchableOpacity onPress={()=>!user ? navigation.navigate('DefinitionsMenu') : null} className="h-[40%] justify-between">
        <Text className='font-dinBold text-white  text-header text-2xl mb-4'>{user ? user.name.toUpperCase() : 'Registar para adicionar nome'}</Text>
        <Text className="font-dinBold text-white opacity-50 text-sm">SÓCIO DESDE</Text>
        <Text className="font-dinBold text-white text-xl">
