...
export const handleGetCalendarIcal = async (): Promise<MatchSchedule> => {
  ...
  const response = await axios.get(
    "https://ics.fixtur.es/v2/sporting-portugal.ics",
  );
  ...
  comp.getAllSubcomponents("vevent").forEach((vevent) => {
    ...
    const dateStr = String(
      event.startDate.toJSDate().toISOString().split("T")[0],
    );
    const yyMM = dateStr.substring(0, 7);
    if (
      Number(yyMM.split("-")[0]) > 2023 ||
      (Number(yyMM.split("-")[0]) >= 2023 && Number(yyMM.split("-")[1]) >= 8)
    ) {
      if (!newItems[yyMM]) {
        newItems[yyMM] = [];
      }
      ...
      if (event.startDate.hour !== 0 && event.startDate.minute !== 0) {
        // don't show all day events time
        newItems[yyMM].push({
          name: event.summary,
          ...
          time:
            Number(Number(event.startDate.hour) + 1) +
            ":" +
