  ...
  <View className="flex h-full rounded-md bg-white_20 justify-end pt-40 items-center ">
    <View className="bg-bgauth  h-full rounded-t-xl  w-full round-md   shadow-xl">
      ...
      >
        <View className="bg-bgauth  h-[390px] rounded-t-xl  w-full round-md  p-6 shadow-xl">
          ...
          <Text className="font-dinLight self-center text-3xl leading-[42px] text-white">
            MATCH
            <Text className="font-dinBold ext-3xl leading-[30px] text-white">
              DAY
            </Text>
          </Text>
          ...
          <GamePrognosis
            ...
            uriAwayLogo={
              teams.data.teamCollection.edges.find(
                (team) => team.node.id === nextGame?.node?.teamAway,
              ).node?.logo
            }
            awayteam={
              teams.data.teamCollection.edges.find(
            ...
            uriLogo={
              teams.data.teamCollection.edges.find(
                (team) => team.node.id === nextGame?.node?.teamHome,
              ).node?.logo
            }
            hometeam={
              teams.data.teamCollection.edges.find(
      ...
      <TouchableOpacity
        ...
        className="flex-row w-full text-center justify-center mt-4  items-center "
      >
        <Text className="text-titleauth text-lg   font-medium">Fechar</Text>
      </TouchableOpacity>
    </View>
  </View>
</Modal>;
